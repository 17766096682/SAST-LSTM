...
class RNN(nn.Module):
    ...
    def forward(self, frames, mask_true):
        ...
        for t in range(self.configs.total_length - 1):
            ...
            for i in range(self.num_layers):
                if i == 0:
                    h_t[0], c_t[0], memory ,g_t[0]= self.cell_list[0](frames_feature, h_t[0], c_t[0], memory,g_t[0])
                else:
                    h_t[i], c_t[i], memory ,g_t[i]= self.cell_list[i](h_t[i - 1], h_t[i], c_t[i], memory,g_t[i])
